...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    slope_more_extreme = sum(abs(sim_slope) >= abs(slope) for sim_slope in slopes) / S  # Replace with code to calculate proportion of slopes more extreme than observed
    intercept_extreme = sum(abs(sim_intercept) >= abs(intercept) for sim_intercept in intercepts) / S  # Replace with code to calculate proportion of intercepts more extreme than observed
...
def index():
    if request.method == "POST":
        ...
        # Return render_template with variables - * changed this
...
def hypothesis_test():
    ...
    fun_message = None
    if p_value <= 0.0001:
        fun_message = "Hark! yee've found the pearl of the ocean" if p_value <= 0.0001 else None
    ...
    plt.axvline(hypothesized_value, color="blue", linestyle="-", label="Hypothesized Slope")  # Add this line
    plt.xlabel("Statistic Value")
    plt.ylabel("Frequency")
    plt.legend()
    plt.title(f"Distribution of Simulated {parameter.capitalize()} with Observed Value")
...
def confidence_interval():
    ...
    # Select the parameter (slope or intercept) to analyze
    if parameter == "slope":
    ...
    # Calculate confidence interval for the parameter estimate using t-distribution
    alpha = 1 - confidence_level / 100
    t_value = t.ppf(1 - alpha / 2, df=S - 1)
    ...
    # Check if the confidence interval includes the true parameter
    ...
    # Plot the confidence interval as a horizontal line in blue
    plt.hlines(0, ci_lower, ci_upper, color="blue", linewidth=3, label=f"{confidence_level}% Confidence Interval")

    # Mark the true parameter with a green dashed line
    plt.axvline(true_param, color="green", linestyle="--", linewidth=2, label="True " + param_name)

    # Set labels, title, and legend
    plt.xlabel(f"{param_name} Estimate")
    plt.ylabel("Density (For Visual Only)")
    plt.title(f"{confidence_level}% Confidence Interval for {param_name} (Mean Estimate)")
